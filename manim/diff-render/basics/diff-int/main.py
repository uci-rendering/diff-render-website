...
class Example_Base(Scene):
    ...
    def get_plot(self, func, clr, theta_txt, dir, smooth=True):
        graph = self.ax.plot(func, x_range=[0., 1., .01 if smooth else .0005], color=clr, use_smoothing=smooth)
...
class FailureExample_0(Example_Base):
    def __init__(self):
        ...
        self.t0 = .5
        self.y0 = .5
        self.f_base = lambda x, y: self.y0 if x < y else .5*self.y0
        self.f0 = lambda x: self.f_base(x, self.t0)
...
class FailureExample_1(FailureExample_0):
    def __init__(self):
        super().__init__()
        self.dt = .1
        self.f1 = lambda x: self.f_base(x, self.t0 + self.dt)

    def construct(self):
        graph0, label0, _, _ = self.get_plot(self.f0, BLUE, r"\theta_0", ORIGIN, smooth=False)
        graph0.set_stroke(opacity = .5)
        label0.next_to(self.ax.c2p(self.t0, .75*self.y0), direction=LEFT)
        self.add(DashedVMobject(graph0, num_dashes=40), label0)

        graph1, label1, area1, area1_label = self.get_plot(self.f1, RED, r"\theta_0 + \Delta\theta", UR*.25, smooth=False)
        label1.next_to(self.ax.c2p(self.t0 + self.dt, .75*self.y0), direction=RIGHT)
        self.add(graph1, label1, area1, area1_label)
...
class FailureExample_2(FailureExample_1):
    ...
    def construct(self):
        graph0, label0, _, _ = self.get_plot(self.f0, BLUE, r"\theta_0", ORIGIN, smooth=False)
        label0.next_to(self.ax.c2p(self.t0, self.y0), direction=DL)
        graph1, label1, _, _ = self.get_plot(self.f1, RED, r"\theta_0 + \Delta\theta", ORIGIN, smooth=False)
        label1.next_to(self.ax.c2p(self.t0 + self.dt, .5*self.y0), direction=DR)
        ...
        line0 = self.ax.get_vertical_line(self.ax.i2gp(self.t0, graph0), color=BLUE).set_opacity(.5)
        line1 = self.ax.get_vertical_line(self.ax.i2gp(self.t0 + self.dt, graph1), color=RED).set_opacity(.5)
